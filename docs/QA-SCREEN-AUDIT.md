...
| Auth — email | `src/screens/onboarding/AuthEmailScreen.tsx` | ✅ | |
| Auth — password | `src/screens/onboarding/AuthPasswordScreen.tsx` | 🔧 | "I forgot my password" was a dead button (`onPress={() => {}}`). No password-reset endpoint exists anywhere in the backend. **Fixed** as a stopgap: tapping it now shows a Notice pointing to the app-store support address. Real fix needs a decision on email provider + backend endpoint + screens — deferred, see [Open items](#open-items). |
| Name | `src/screens/onboarding/NameScreen.tsx` | ✅ | |
| About you (profile) | `src/screens/onboarding/ProfileScreen.tsx` | ✅ | |
...
| Insights | `src/screens/insights/InsightsScreen.tsx` (+ `WeekChart.tsx`, `WeekReviewCard.tsx`) | 🔧 | The weekly Calories/Protein/Fibre charts always drew their rightmost bar as "Today" (full-opacity gradient fill, bold day-initial) and the legend always read "Today"/"Earlier" — even after paging back to a past week with the header's chevron, where the rightmost bar is just the last day of *that* week, not today. **Fixed**: `WeekChart` takes a `highlightLast` prop (only the current week passes `true`), and `ChartLegend` takes a `currentLabel` that becomes "Most recent" for a past week instead of "Today". Verified: `tsc --noEmit` clean, full suite passes. |
