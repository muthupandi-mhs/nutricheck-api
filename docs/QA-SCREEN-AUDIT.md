...
| Name | `src/screens/onboarding/NameScreen.tsx` | 🔧 | Externally redesigned mid-session to the same rising-bottom-sheet scaffold `AuthStep` uses (scrim, spring-in panel, rounded top, handle), later gaining a brand mark in the backdrop and a hardware-back confirmation dialog. It still used the boxed/labelled `FormField` for its two inputs though, not the pill-shaped `AuthFormField` the login screens use, so it didn't fully match. **Fixed**: switched both fields to `AuthFormField` and fixed the same stuck-disabled-Continue bug found on the email/password steps. Separately, reported by the user with a screenshot: on some Android phones the Continue button sat mostly hidden under the keyboard — the keyboard-height event some Android keyboards send doesn't always include an autofill/suggestion strip drawn above the keys, so the panel's keyboard-clearance margin under-shifted it, and `scrollMaxHeight` was a flat 64% of the whole window with no keyboard-awareness at all. **Fixed**: `scrollMaxHeight` is now anchored to the actual space left above the keyboard, and the clearance adds a small Android-only safety margin over the reported height. No behavior change on iOS. |
...
19. **(ops, not code)** The local Docker API/worker containers were rebuilt and restarted — they were running a Sep 1 image, predating `DELETE /v1/me` (and other recent backend work), which is why "Delete account" was returning a 404 "Cannot DELETE /v1/me". No source bug; the route was correct and just wasn't running yet.
20. `src/screens/onboarding/NameScreen.tsx` — the Continue button no longer sits partly hidden under the keyboard on Android phones whose keyboard draws an autofill/suggestion strip that isn't always included in the reported keyboard height; the scrollable area now shrinks based on actual space above the keyboard instead of a flat share of the whole screen.
